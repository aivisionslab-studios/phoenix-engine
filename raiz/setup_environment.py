...
class ModelManager:
    ...
    def _get_models_base_dir(self) -> Path:
        # PHX-FIX (auditoria 2026-08-09): esta função tinha sua própria
        # resolução de workspace duplicada e paralela à do
        # PhoenixPaths/InstallTargetSelector - e, ao contrário deles, não
        # fazia nenhum scan de disco: lia "C:/ProgramData/Phoenix/storage.json"
        # e, se falhasse por qualquer motivo (silenciado por um "except: pass"
        # genérico), caía num fallback fixo em "E:/Phoenix/Models". Numa
        # máquina sem drive E:, ou com o workspace real em outro disco (o
        # próprio propósito do InstallTargetSelector é escolher o disco
        # certo), isso resolvia silenciosamente pra um caminho que nunca
        # existiu. PhoenixPaths.get_models_base() já resolve isso direito -
        # sem nenhuma letra de drive fixa, lendo o manifest real gravado
        # pelo instalador - então delega pra lá em vez de duplicar a lógica.
        from phoenix_kernel.paths import PhoenixPaths
        return PhoenixPaths.get_models_base()
