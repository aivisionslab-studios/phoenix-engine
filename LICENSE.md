...
## Nota sobre dependências de terceiros (binários externos)

Alguns componentes do Phoenix invocam binários de terceiros como
processos externos via `subprocess` (mesmo padrão para llama.cpp,
stable-diffusion.cpp e Piper) — o código-fonte desses binários não é
distribuído nem incorporado neste repositório, e nenhuma licença deles se
aplica ao código do Phoenix em si.

- **Piper (TTS neural local)**: o binário é obtido de
  `github.com/OHF-Voice/piper1-gpl`, sob licença **GPL-3.0** (o projeto
  original, `rhasspy/piper`, era MIT e foi arquivado em out/2025). Como o
  Phoenix apenas invoca o binário já compilado como processo externo
  (stdin/stdout), sem link/embedding de código, esta licença GPL-3.0 se
  aplica somente ao binário do Piper, não ao restante do projeto. Esta
  análise é uma opinião técnica, não aconselhamento jurídico — para uso
  comercial, vale confirmar com um advogado.
